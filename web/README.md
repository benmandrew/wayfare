...
Three things follow from holding a set rather than one archive. `minZoom` is the deepest floor of the set, the maximum of the loaded headers, because at a zoom one archive cannot answer, that region alone goes blank. Feature ids are unique within an archive and not across archives, so hover highlighting is keyed on the source as well as the id. And where two archives genuinely overlap — Northern Ireland and the Republic were matched against one island graph, so every cross-border road is in each — the road is still *drawn* twice, but neither the match count nor the info card treats it as two roads. The count keys on the OpenStreetMap (OSM) way id, which is the one identity two archives share. The card merges their service lists: one feature from each archive, then the route numbers unioned and the totals summed. Below the *detail zoom* the tiles carry no way id, so the count falls back to the per-archive id there.
