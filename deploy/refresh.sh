...
# file. One region per invocation, because one data root holds one feed version.
#
#   ./deploy/refresh.sh
#   REFRESH_NO_FETCH=1 ./deploy/refresh.sh    # retry without re-downloading
#   REFRESH_SERVICE=wayfare-ni ./deploy/refresh.sh   # a second region in one project
#
# REFRESH_SERVICE names the Compose service to run the stages as, and exists
# because a Compose project is a Valhalla graph rather than a region. Ireland and
# Northern Ireland are matched against one island graph, so they are two services
# and two data roots sharing one project and one valhalla container, and running
# both means invoking this twice with the service changed. Great Britain has a
# graph to itself and never sets it.
#
...
wayfare() { docker compose run --rm -T "${REFRESH_SERVICE:-wayfare}" "$@"; }
